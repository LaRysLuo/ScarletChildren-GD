...
func pick_map_scene():
	...
	scene_picker.add_filter("*.tscn","RPG配置的演出文件")
...
func from_data(data:BaseEventNode) -> BaseGN:
	if data is CinemaData:
		map_path = data.cinema_scene_path
		refresh_picker_info()
	return self

func to_data(edit:GraphEdit) -> BaseEventNode:
	return CinemaData.new(BaseEventNode.CinemaMode,self.position_offset,map_path)
