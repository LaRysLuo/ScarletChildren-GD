...
signal on_ui_pressed(key:int)

## INFO 功能说明
# 这里主要作用是手柄的防抖
func _input(event: InputEvent) -> void:
	if GameManager.game_state.is_buszing(): return
	if !input_trigger && event.is_pressed():
		...
		if key == -1: return
		if GameManager.game_state.is_uionly():
			on_ui_pressed.emit(key)
		else: on_action_pressed.emit(key)
